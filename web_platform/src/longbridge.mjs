import {AppError,requireValue,nowISO} from './engine.mjs';
...
export async function signedHeaders(credentials,path,query='',timestamp=String(Math.floor(Date.now()/1000))) {
  ...
  const canonical=`GET|${path}|${query}|${values}|${signed}|`;
...
export async function connectionStatus(env,db){
  const row=await db.prepare('SELECT updated_at FROM longbridge_connection WHERE id=1').first();
  return {ok:true,configured:Boolean(row),storage_ready:Boolean(env.BROKER_CREDENTIAL_KEY),updated_at:row?.updated_at||null,execution_enabled:false,environment_verified:false};
}
...
export async function saveConnection(env,db,user,input,auditStatement){
  ...
  return {ok:true,configured:true,updated_at:timestamp,execution_enabled:false,environment_verified:false,account,message:'凭证已加密保存，账户资金查询成功。请与长桥模拟账户核对；当前仅查询，不会下单。'};
}
...
export async function connectionOverview(env,db){
  ...
  const out={ok:true,source:'Longbridge OpenAPI',environment:'用户声明模拟账户（待核验）',environment_verified:false,execution_enabled:false,fetched_at:nowISO(),errors:{}};
