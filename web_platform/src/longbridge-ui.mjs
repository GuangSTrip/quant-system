export function createLongbridgePanel(api){
  ...
  async function refresh(){
    ...
    try{
      ...
      try{const d=await api('longbridge/overview');if(current!==generation)return;render(d);say((d.stale?'本次刷新未完成，保留上次成功结果':d.ok?'查询成功':'部分查询失败')+' · '+d.fetched_at+'。请与长桥模拟账户核对，交易授权与委托请查看下方长桥模拟交易区。');}
