...
function bridgeConfig(env){
  ...
  const local=env.MYQUANT_BRIDGE_LOCAL_ONLY==='true'&&raw==='http://127.0.0.1:8765';
  requireValue((base.protocol==='https:'||local)&&base.username===''&&base.password===''&&base.search===''&&base.hash===''&&base.pathname==='/', 'A股桥接必须使用无凭据的 HTTPS 地址，或显式启用固定本机桥接',503,'MYQUANT_BRIDGE_INVALID');
  return {base,secret};
}

function targetFor(base,path){
  requireValue(/^\/v1\/(?:health|status|orders(?:\/preview|\/cancel)?|reconcile|control|audit|ledger)$/.test(path),'A股桥接路径无效',500,'MYQUANT_BRIDGE_PATH');
