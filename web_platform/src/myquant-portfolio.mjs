...
export const cnPortfolioAdapter={
 ...
 async ledger(db,s,env){const r=await myquantBridge(env,'/v1/ledger',{actor:'portfolio:'+s.run_id});return r.orders.map(o=>({key:o.client_id.replaceAll('-',''),id:o.client_id,broker_id:o.broker?.order_id||null,status:o.status,qty:Number(o.request.quantity),submitted_at:o.created_at,updated_at:o.updated_at,filled_at:null,symbol:cnSymbol(o.request.symbol),side:o.request.side,filled:Number(o.broker?.filled_volume||0),price:Number(o.broker?.filled_vwap||0)}));},
