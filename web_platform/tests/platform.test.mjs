...
test('account data and writes require login; writes additionally require a trusted origin',async t=>{
  const {request,broker}=setup(t);for(const path of ['/api/v1/overview','/api/paper/status','/api/v1/equity'])assert.equal((await request(path,undefined,{auth:false})).status,401);assert.equal(broker.calls.length,0);const overview=await request('/api/v1/overview');assert.equal(overview.status,200);assert.equal(overview.data.account.equity,'100000');assert.equal(overview.data.control.halted,true);assert.ok(!JSON.stringify(overview.data).includes('test-paper'));
