...
try {
 ...
 foreach($network in @('10.251.0.0','10.252.0.0','10.253.0.0')){
  $prefix=$network+'/16'
 ...
 @{at=(Get-Date).ToUniversalTime().ToString('o');interface=$address.InterfaceAlias;gateway=$gateway.NextHop;allowed=@('10.250.0.0/16','10.251.0.0/16','10.252.0.0/16','10.253.0.0/16')} | ConvertTo-Json | Set-Content -LiteralPath (Join-Path $PSScriptRoot '../.lan/campus-network-repair.json') -Encoding UTF8
} catch {
