...
面向单账户美股／ETF的日频自动量化模拟交易系统。网页读取 Alpaca Paper 账户和 IEX 行情；服务器实际执行模拟订单、撤单、风控和对账。原 Python 多资产回测保留为独立研究基线。
...
9. 要演示自动量化：在「自动策略」选择已保存的回测、设置预算、确认心跳后启动；关闭网页，等待后台记录，重新打开查看决策、订单和成交回报。日频策略不保证每轮有订单。
...
| 自动策略 | 保存回测后授权启动；后台日线信号、自动限价单、成交跟踪、暂停恢复和运行证据；关闭网页继续运行 |
...
详见 [COURSE_ACCEPTANCE.md](COURSE_ACCEPTANCE.md)。本次课程交付范围为单账户、单个预置日频策略的自动 Paper 交易；使用流程见 [AUTOMATION.md](AUTOMATION.md)。
