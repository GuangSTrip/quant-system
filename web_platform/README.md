...
网页「港股 · 长桥」支持加密保存模拟账户凭证、资金和持仓查询、限价下单、撤单、成交核对，以及独立的定时定额限价买入策略。长桥使用 HKD 预算与独立订单账本；不改变原有 Alpaca Paper 策略与订单。详见 [长桥接入与验收](LONGBRIDGE.md)。

「日线策略研究」保留 main 的三市场选股、组合研究及历史展示，尚未自动连接长桥或 Alpaca 下单。长桥定投执行器不等同于日线组合策略，研究结果不能直接作为今日委托。

富途只读桥接保留供历史兼容，不再显示在侧栏；相关配置见 [旧富途接入说明](HK_INTEGRATION.md)。
