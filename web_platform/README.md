...
新增「港股模拟账户」页面，通过富途 OpenD 桥接读取实际模拟账户资金、持仓、订单和报价。需要独立配置桥接服务；尚未开放港股下单与自动策略，也未完成实际 OpenD 联调。部署步骤和边界见 [港股接入说明](HK_INTEGRATION.md)。


## 长桥港股连接

网页「港股 · 长桥」支持由课程操作员输入模拟账户三项凭证，验证资金读取后加密保存，再查询港股持仓及当日订单。无需 OpenD。当前仅只读，尚未提供港股交易执行；配置与边界见 [长桥连接说明](LONGBRIDGE.md)。

长桥港股已提供独立的限价下单、撤单、成交核对与定时限价定投策略；操作方法、测试范围和真实交易验收限制见 [LONGBRIDGE.md](LONGBRIDGE.md)。
