...
class AshareOrder:
    ...
    def from_payload(cls, payload: Mapping[str, Any]) -> "AshareOrder":
        ...
        if order_type == "limit":
            ...
            if not math.isfinite(limit_price) or limit_price <= 0 or limit_price > 1_000_000:
...
class MyQuantBridge:
    ...
    def __init__(
        ...
        if not math.isfinite(self.max_order_notional) or self.max_order_notional <= 0:
            raise ValueError("单笔金额上限必须是有限正数")
    ...
    def start(self, context: Any, host: str = "127.0.0.1", port: int = 8765, start_server: bool = True) -> None:
        ...
        self.store.set_halted(True, "桥接启动后需重新对账恢复", "bridge")
    ...
    def refresh(self, context: Any) -> Dict[str, Any]:
        try:
            ...
            self._ingest_native(all_orders, "bridge")
            return snapshot
    ...
    def _available_cash(self) -> Optional[float]:
        ...
        for key in ("available", "available_cash"):
            try:
    ...
    def preview(self, payload: Mapping[str, Any], _actor: str) -> Dict[str, Any]:
        ...
        if not getattr(self.gm, "paper_environment_verified", False):
            raise BridgeError("仿真服务环境未验证，拒绝新增订单", 409, "PAPER_ENVIRONMENT_UNVERIFIED")
        control = self.store.control()
        ...
        if order.side == "buy" and notional is not None:
            ...
            if available is None:
                raise BridgeError("缺少有效可用资金，拒绝下单", 422, "CASH_UNAVAILABLE")
    ...
    def _submit_on_strategy_thread(self, raw: Mapping[str, Any], actor: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not result or not result.get("cl_ord_id") or status == "unknown":
                raise RuntimeError("掘金未返回可确认的订单回执")
