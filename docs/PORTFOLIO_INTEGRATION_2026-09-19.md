...
- Python 53 项及旧富途桥接 4 项通过，包含 178 个注册组合逐项对照合并前 `e4648b1` 的确定性研究结果；固定种子 560 日/40 证券，覆盖成本、落选、通道状态和风险分配。完整原始结果在 `tests/fixtures/portfolio_original_golden.json`。逐字段比较保留结构与整数精确一致，浮点容限为 `rtol=1e-10, atol=1e-8`，避免平台浮点尾数触发哈希误报。
